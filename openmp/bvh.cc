...
inline void getBBAndBBCentroid(Mesh const &mesh, std::vector<unsigned int> &faceIDs, AABB &bb, AABB &bbCentroid) {
	for (size_t i = 0; i < faceIDs.size(); ++i) {
		...
		Vec3f centroid = tri.getCentroid(); /* Get the triangle's centroid and add it */
		bbCentroid.merge(centroid);			/* to the BV (it might increase in size) */
		bb.merge(tri.getAABB());				/* Merge the AABBs so the tree rebuilds */
	}
}
/*
...
//	std::cout << "\rCutting..." << std::flush;
	...
	/* I managed to enance it mathematically, */
	/* but I don't see why we do this.   ;â©;  */
	/******************************************/
	...
	for (size_t i = 0; i < faceIDs.size(); ++i) {
		Triangle tri(&mesh, faceIDs[i]);		/* Get the triangle from the mesh */
		Vec3f centroid = tri.getCentroid();		/* Get the current centroid */
...
void BVH::buildBVH(Mesh const &mesh) {
	...
	std::vector<unsigned int> faceIDs(size);
...
// 	this->triangles.reserve(size);
	this->nodes = std::vector<unsigned int>(size * 2 - 1);
 ...
 */
unsigned int const BVH::build(const Mesh &mesh, std::vector< unsigned int > &faceIDs, unsigned int &ind) {
...
// 	cl_uint n;
// 	this->nodes.at(ind) = n;
	unsigned int & node = this->nodes.at(ind);
...
void BVH::cutFacesSAH(const Mesh &mesh, std::vector<unsigned int> &faceIDs, std::vector<unsigned int> &leftIDs, std::vector<unsigned int> &rightIDs, AABB &bb) {
	...
	float cBV2 = 1.0; 			// traversal_cost
	float cObj = 1.0;			// intersection_cost

	float SACurrent = getSurfaceArea(bb);	// SA of the current node
