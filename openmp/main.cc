...
int main(int argc, char **argv) {
	...
	// Build the kernel
	{
		...
		std::vector<unsigned int> sortedFaces;
		sortedFaces.reserve(mesh.faces.size());
		for (size_t i = 0; i < bvh.triangles.size(); ++i) {
			unsigned int faceID = bvh.triangles[i] * 3;
...
// 	float * tmp = (float*)malloc(rt.totalWidth * rt.totalHeight * sizeof(float));
	// Execute
	{
		Timer timer;
		bool success = host(mesh, bvh.nodes, bvh.aabbs, tmp);
